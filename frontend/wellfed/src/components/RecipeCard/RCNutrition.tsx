...
const NutritionCard: React.FC<NutritionProps> = ({ calories, protein, fat, carbohydrates, recipeId }) => {
  ...
  const handleAddToCalendar = () => {
    router.push('/calendar-section/add-event'); // Navigate to the desired route
  };
  ...
  return (
    <div className="mx-auto bg-white p-4">
      ...
      <div className="flex justify-center items-center mb-4">
        <button
          onClick={() => router.push(`${recipeId}/recipe-steps`)}
          className="bg-gradient-to-r from-[#B64B29] to-[#EC9556] hover:from-[#AD3711] hover:to-[#FFB54D] text-white font-bold py-3 px-10 rounded-full text-[18px] shadow-lg transition duration-300 ease-in-out"
          style={{ width: '358px' }}
        >
          Start Cooking
        </button>
      ...
      <div className="flex justify-center items-center">
        <button
          ...
          className="relative bg-white hover:bg-[#DADADA] font-bold py-2 px-4 rounded-full border-2 transition duration-300 ease-in-out text-lg shadow-lg"
          style={{ width: '358px' }}
        >
          <span
            className="bg-clip-text text-transparent text-[18px]"
            style={{
              backgroundImage: 'linear-gradient(to right, #B64B29, #EC9556)',
